...
class GeradorDeBilhetes:
    ...
    def gerar_sequencia_unica(self):
        ...
        # Adiciona um contador de segurança para evitar loops infinitos
        max_attempts = 1000
        attempts = 0
    ...
    def draw_ticket_on_a4_slot(self, draw, slot_index, bilhete_contador, date_str, ticket_properties):
    ...
    def generate_pdf_file(self, num_pages, date_str, start_number, file_path, ticket_properties):
        """Gera um único PDF com os bilhetes."""
        pdf = FPDF(unit="pt", format="A4", orientation='L')
        pdf.set_auto_page_break(False)

        total_bilhetes_file = num_pages * CARTELAS_POR_PAGINA
        ...
        for page_num in range(num_pages):
            ...
            for i in range(CARTELAS_POR_PAGINA):
                self.draw_ticket_on_a4_slot(draw, i, bilhete_contador, date_str, ticket_properties)
                bilhete_contador += 1
    ...
    def start_multi_file_generation(self):
        ...
        save_dir = filedialog.askdirectory(title="Selecione a Pasta para Salvar os PDFs")
        if not save_dir:
            return
    ...
    def generation_worker(self, num_files, num_pages, date_str, start_number, save_dir):
        """Lógica que roda em uma thread separada para gerar os arquivos."""
        ...
        try:
            for file_num in range(1, num_files + 1):
                ...
                # Verifica se o arquivo anterior terminou no número correto
                file_start_number = current_ticket_number

                self.update_status(f"Iniciando PDF {file_num}/{num_files}...")

                self.generate_pdf_file(num_pages, date_str, file_start_number, file_path, ticket_properties)
                
                # Atualiza o número inicial para o próximo arquivo
                current_ticket_number = file_start_number + (num_pages * CARTELAS_POR_PAGINA)
                
            self.finalize_generation(True, f"✅ Sucesso! {num_files} PDFs gerados na pasta:\n{save_dir}")
    ...
    def setup_ui(self):
        ...
        control_frame = ctk.CTkFrame(progress_frame, fg_color="transparent")
