...
CAMPO_COORDENADAS = {
    'campo1': (513, 358),  # Leve ajuste para baixo no campo 1
    'campo2': (513, 428),  # Leve ajuste para baixo no campo 2
    'campo3': (513, 494),  # Leve ajuste para baixo no campo 3
    'numero_bilhete': (2, 490), # Leve ajuste para baixo no número
    'data_bilhete': (232, 490)
}
...
BILHETE_ORIGINAL_LARGURA_PX = 1200
BILHETE_ORIGINAL_ALTURA_PX = 700

# Dimensões da página A4 em pontos (Paisagem: Largura x Altura)
PAPER_WIDTH_PT = 841.89
PAPER_HEIGHT_PT = 595.28

# MUDANÇA: Define a grade para 4 colunas por 5 linhas, totalizando 20 bilhetes por página
GRID_COLS = 4  
GRID_ROWS = 5
CARTELAS_POR_PAGINA = GRID_COLS * GRID_ROWS # Agora é 20

# MUDANÇA: Reduz a margem de segurança para deixar os bilhetes mais próximos da borda (de 20 para 5 pontos)
SAFE_MARGIN_PT = 5
...
# CÁLCULO DE DIMENSIONAMENTO ROBUSTO PARA GARANTIR QUE 4x5 CAIBA NO A4 PAISAGEM
PROPORTION_BILHETE = BILHETE_ORIGINAL_LARGURA_PX / BILHETE_ORIGINAL_ALTURA_PX

# 1. Calcula o espaço útil total
USABLE_WIDTH_PT = PAPER_WIDTH_PT - (2 * SAFE_MARGIN_PT)
USABLE_HEIGHT_PT = PAPER_HEIGHT_PT - (2 * SAFE_MARGIN_PT)

# 2. Calcula a largura e altura máximas permitidas por bilhete para a grade 4x5
MAX_W_PER_TICKET = USABLE_WIDTH_PT / GRID_COLS
MAX_H_PER_TICKET = USABLE_HEIGHT_PT / GRID_ROWS

# 3. Determina o fator limitante (manter a proporção)
# Se a largura máxima por bilhete (dividida pela proporção) for menor que a altura máxima, a largura é o limitante.
if (MAX_W_PER_TICKET / PROPORTION_BILHETE) <= MAX_H_PER_TICKET:
    # A largura é o fator limitante
    BILHETE_RENDER_LARGURA = MAX_W_PER_TICKET
    BILHETE_RENDER_ALTURA = BILHETE_RENDER_LARGURA / PROPORTION_BILHETE
else:
    # A altura é o fator limitante (necessário para a grade 4x5)
    BILHETE_RENDER_ALTURA = MAX_H_PER_TICKET
    BILHETE_RENDER_LARGURA = BILHETE_RENDER_ALTURA * PROPORTION_BILHETE
...
# Centraliza a grade, mas agora usando a margem de segurança mínima
MARGIN_X = (PAPER_WIDTH_PT - TOTAL_RENDER_WIDTH) / 2
...
def generate_and_save_pdf_threaded(num_pages, date_str, start_number, app_instance, progress_bar, btn_gerar):
    ...
    # Inicia a contagem com o número inicial fornecido
...
def start_generation_in_thread():
    ...
    # LÓGICA: Obtém o número inicial do bilhete
    try:
...
# Classes de PDF que estavam sobrando foram removidas para manter o arquivo limpo.
